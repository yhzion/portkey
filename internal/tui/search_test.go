...
func TestSearch_SlashKey_ActivatesSearch(t *testing.T) {
	...
	if m.searchQuery != "" {
		t.Errorf("searchQuery = %q, want empty on activation", m.searchQuery)
	}
}
...
func TestSearch_TypeChars_AppendsToQuery(t *testing.T) {
	...
	if m.searchQuery != "pro" {
		t.Errorf("searchQuery = %q, want %q", m.searchQuery, "pro")
	}
}
...
func TestSearch_Backspace_RemovesLastChar(t *testing.T) {
	...
	m.Update(tea.KeyMsg{Type: tea.KeyBackspace})
	if m.searchQuery != "p" {
		t.Errorf("searchQuery = %q, want %q", m.searchQuery, "p")
	}
}
...
func TestSearch_FiltersResults(t *testing.T) {
	...
	if len(m.filtered) != 2 {
		t.Errorf("len(filtered) = %d, want 2 (production-api, prod)", len(m.filtered))
	}
}
...
func TestSearch_NoMatches(t *testing.T) {
	...
	if len(m.filtered) != 0 {
		t.Errorf("len(filtered) = %d, want 0 for no match", len(m.filtered))
	}
}
...
func TestSearch_NumberOutOfRange_AppendsToQuery(t *testing.T) {
	...
	if m.searchQuery != "9" {
		t.Errorf("searchQuery = %q, want %q (number out of range should append)", m.searchQuery, "9")
	}
}
